...
void solve() {
	...
	for (ll bit = 1 << 29 ; bit > 0 ; bit >>= 1) {
		bool poss = true;
		for (ll i = 0; i < n; i++) {
			if (state[i] == 0 and !(A[i]&bit))poss = false;
			else if (state[i] == 1 and !(B[i]&bit))poss = false;
			else if (!(A[i]&bit) and !(B[i]&bit))poss = false;
		}
		if (!poss)continue;
		for (ll i = 0; i < n; i++) {
